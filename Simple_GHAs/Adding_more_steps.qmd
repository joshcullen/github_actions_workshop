...
jobs:  
  install_inla: 
    ...
    - name: Install INLA
      shell: Rscript {0}  # <4>
      run: |
        remotes::install_version("INLA", version = "25.06.13", repos = c(getOption("repos"), INLA = "https://inla.r-inla-download.org/R/testing"), dep = TRUE)  # <5>
    
    - name: Check INLA version
      shell: Rscript {0}
      run: INLA::inla.version()  # <6>
```
1. Now using `macos-latest` runner (as different example)
2. Needs to be compatible w/ selected INLA version
3. Need to install `remotes` R package to help with installation of `INLA`
4. Specifying the shell as R code, which allows me to directly run R code (or scripts)
5. Function from [INLA](https://www.r-inla.org/download-install) for package installation
6. Check that INLA was succesfully installed by checking version with built-in function (in R)
:::
