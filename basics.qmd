...
::: {.callout-note}
GitHub Actions run in the cloud on a fresh virtual machine (VM) each time it's triggered. While this is very useful for ensuring code reproducibility, the lack of persistent storage means that all software and files need to be installed or loaded every time prior to performing any tasks. Keep this in mind when creating your own workflows and treat your GitHub repo (or some external database or cloud bucket) as your sources of file storage since the VM will be wiped at the completion of every GitHub Actions workflow.
:::
...
Additionally, you'll receive an automated email if a GitHub Action run fails and it will direct you to the location in the GitHub Action log where the error occurred. These many different features make it much easier for automating tasks and fixing issues as part of your research or software development workflow.
...
While not covered in during this workshop, it is also worth mentioning that users with a GitHub Enterprise Cloud plan can use what are called **large runners**, which provide a much wider range of high-performance features compared to standard GitHub runners. Some relevant features provided by **large runners** include:

- More memory, cores, and storage
- Autoscaling to support concurrent workflows
- GPU-powered runners

Different features and sizes for these runners are available depending on the selected operating system, where macOS **large runners** generally have more limitations than Linux or Windows.

:::{.callout-note}
For both private and public repositories, when larger runners are in use, they will always be billed at the per-minute rate. However, there is no cost associated with creating a larger runner that is not being used by a workflow.
:::

For more information on **large runners**, please refer to these GitHub [documentation](https://docs.github.com/en/enterprise-cloud@latest/actions/concepts/runners/larger-runners) and [reference](https://docs.github.com/en/enterprise-cloud@latest/actions/reference/runners/larger-runners) pages.