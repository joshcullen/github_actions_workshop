...
other-links:
  - text: "bash_debug.yml"
    href: "https://github.com/joshcullen/github_actions_workshop/actions/workflows/bash_debug.yml"
    icon: "github"
  - text: "copernicusmarine_install.yml"
    href: "https://github.com/joshcullen/github_actions_workshop/actions/workflows/copernicusmarine_install.yml"
    icon: "github"
  - text: "gdal_cdo_install.yml"
    href: "https://github.com/joshcullen/github_actions_workshop/actions/workflows/gdal_cdo_install.yml"
    icon: "github"
  - text: "inla_install.yml"
    href: "https://github.com/joshcullen/github_actions_workshop/actions/workflows/inla_install.yml"
    icon: "github"
---
...
5. Allow the `setup-r-dependencies` action to auto-detect which packages (and versions) to install from the R scripts (or Quarto docs) that are included in your directory
...
Below, I'll show how to install these different sources of software and check that they have installed properly. **Please refer to the "[Use secrets in a workflow](../Complex_GHAs/Run_scripts.qmd#use-secrets-in-a-workflow)" section for examples showing how to use these tools within a GitHub Actions workflow since this section is focusing on the basics of setting up a virtual machine to perform tasks.**
...
### GDAL, CDO, and other geospatial tools
  ...
  install_gdal_cdo: 
    ...
    - name: Install geospatial libraries
      run: |
        ...
        sudo apt-get install libudunits2-dev gdal-bin libgdal-dev libgeos-dev libproj-dev libsqlite3-dev  # <3>
    
    - name: Install CDO
      run: sudo apt-get install cdo  # <4>
...
2. Update other necessary software on `runner`
3. Command to install all relevant geospatial libraries
4. Command to install `cdo`
:::
...
::: {.annot-2col}
```{.yaml filename="inla_install.yml"}
name: Install INLA
...
jobs:  
  install_inla: 
    ...
    - name: Install INLA
      shell: Rscript {0}  # <5>
      run: |
        remotes::install_version("INLA", version = "25.06.13", repos = c(getOption("repos"), INLA = "https://inla.r-inla-download.org/R/testing"), dep = TRUE)  # <6>
    
    - name: Check INLA version
      shell: Rscript {0}
      run: INLA::inla.version()  # <7>
```
1. Now using `macos-latest` runner (as different example)
2. Needs to be compatible w/ selected INLA version
3. Need to install `remotes` R package to help with installation of `INLA`. The `any::` syntax is used to specify packages to download from CRAN.
4. Also need to install `sf` and `terra` as dependencies for `INLA`
5. Specifying the shell as R code, which allows me to directly run R code (or scripts)
6. Function from [INLA](https://www.r-inla.org/download-install) for package installation
7. Check that INLA was succesfully installed by checking version with built-in function (in R)
:::
